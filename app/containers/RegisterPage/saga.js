...
export function* handleName() {
  ...
  if (!name) return yield put(enterNameErrorAction('empty'));
  if (name.trim().length > limit || !isString.test(name))
    return yield put(enterNameErrorAction('error'));
...
export function* handleSurname() {
  ...
  if (!surname) return yield put(enterSurnameErrorAction('empty'));
  if (surname.length > limit || !isString.test(surname))
    return yield put(enterSurnameErrorAction('error'));
...
export function* loadCurrency() {
  ...
  try {
    ...
    if (!response) return yield put(loadCurrencyErrorAction('error'));
...
export function* handleCurrency() {
  ...
  if (!currencyId) return yield put(enterCurrencyErrorAction('empty'));
...
export function* handleEmail() {
  ...
  if (!email && !isDataProcessingAgreement) {
    return (
      ...
      yield put(errorDataProcessingAgreementAction('isDataProcessingAgreement'))
    );
  }
  if (!email) return yield put(enterEmailErrorAction('empty'));
  if (!isEmail.test(email) || email.length > limit)
    return yield put(enterEmailErrorAction('error'));
  if (!isDataProcessingAgreement)
    return yield put(
      errorDataProcessingAgreementAction('isDataProcessingAgreement'),
    );

  try {
    const response = yield call(request, requestURL);
    if (response.isEmail) return yield put(enterEmailErrorAction('exist'));

    yield put(enterEmailSuccessAction());
    yield call(registerAttempt);
  } catch (err) {
    yield put(enterEmailErrorAction(err));
  }
}
