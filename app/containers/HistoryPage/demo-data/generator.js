...
export const employeeTaskValues = {
  ...
  dueDate: ({ random, record }) =>
    generateDate({
      ...
      year: 2016,
      month: () =>
        Math.floor(random() * 2) + new Date(record.startDate).getMonth(),
    }),
};
...
export const testBank = {
  date: ['02-03-2019, 20:20', '01-02-2019, 16:28'],
  sender: ['Adrian Pietrzak', 'Michał Bortkiewicz'],
  recipient: ['Adrian Pietrzak', 'Michał Bortkiewicz'],
  units: ({ random }) => Math.floor(random() * 4) + 1,
  transferTitle: ['irek kiedy fajrant xD', 'test przykladowy tytul'],
  amount: ({ random }) => Math.floor(random() * 10000) + 1000,
  discount: ({ random }) => Math.round(random() * 0.5 * 1000) / 1000,
  saleDate: ({ random }) =>
    generateDate({
